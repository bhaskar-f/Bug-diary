...
function SideNavLink({ to, icon, label, end = false }) {
  return (
    <NavLink
      ...
      className={({ isActive }) =>
        `group relative overflow-hidden w-auto lg:w-full font-medium leading-none text-[0.95rem] flex items-center gap-1.5 p-2.5 rounded-lg transition-colors duration-200 ${
...
export default function Leftnav({ removetoken, username, email }) {
  ...
  return (
    <div className="navbar hidden md:flex md:sticky md:top-0 md:z-40 lg:static w-full lg:w-56 xl:w-72 lg:min-h-screen lg:max-h-screen px-3 py-3.5 sm:px-4 lg:px-5 lg:pt-3.5 flex-row items-center gap-2.5 lg:flex-col lg:items-stretch lg:gap-0 border-b border-zinc-300 lg:border-b-0 lg:border-r bg-white">
      ...
      <hr className="-mx-3 hrline hidden lg:block sm:-mx-4 lg:-mx-5 mt-2.5 w-auto text-zinc-100" />
      <div className="min-w-0 ml-3 sm:ml-4 lg:ml-0 lg:w-full lg:mt-3.5 flex items-center gap-1.5 overflow-x-auto lg:overflow-visible lg:flex-wrap">
